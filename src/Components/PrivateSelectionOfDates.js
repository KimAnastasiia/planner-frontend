...
let PrivateSelectionOfDates = () => {
    ...
    let getMeetingInfo = async (arrayOfDates) => {
        let response = await fetch(Commons.baseUrl + `/meetings?meetingId=${id}&token=${token}&access_token=${localStorage.getItem("access_token")}`)
        if (response.ok) {
            let data = await response.json()
            console.log(data)
    ...
    let addParticipation = async (time) => {
        let response = await fetch(Commons.baseUrl +`/participation?access_token=${localStorage.getItem("access_token")}`, {
            method: "POST",
            headers: {
            ...
            body: JSON.stringify({
                ...
                time: {id: time},
                userToken:token,
                meetingToken:meetingDataRef.current.token
            })
        })
        if (response.ok) {
    ...
    let deleteParticipation=async(time)=>{
        ...
        let response = await fetch(Commons.baseUrl + `/participation/${time}?access_token=${localStorage.getItem("access_token")}`, {
            method: 'DELETE'
        })
    }
            ...
            <Flex vertical align='center' justify='center' style={{ width: "100%" }}>
                ...
                {voted && 
                    <Flex vertical style={{ border: "1px solid #D3DCE3",backgroundColor: "white", borderRadius: 10, padding: 10, marginBottom:10}}>
                        ...
                        <Text >Now it depends on the organizer. We will send you an email, when will the selected final hour.</Text>
...
export default PrivateSelectionOfDates